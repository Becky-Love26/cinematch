 ...
 * @author aluno
 */
public class InserirTipoElenco extends javax.swing.JFrame {

    public InserirTipoElenco() {
        initComponents();
        carregarCombos();
    }
    ...
    private void carregarCombos() {
        try {
            ...
            // Limpar combos antes de carregar
            jComboBoxElenco.removeAllItems();
            jComboBoxFilme.removeAllItems();
            jComboBoxTipoElenco.removeAllItems();
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(layout.createSequentialGroup()
                        ...
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                            .addComponent(jComboBoxElenco, 0, 350, Short.MAX_VALUE)
                            .addComponent(jComboBoxFilme, 0, 350, Short.MAX_VALUE)
                            .addComponent(jComboBoxTipoElenco, 0, 350, Short.MAX_VALUE)))
                    .addGroup(layout.createSequentialGroup()
    ...
    private void btnInserirActionPerformed(java.awt.event.ActionEvent evt) {
        // Verifica se algum campo está vazio ou selecionou a opção padrão
        if (jComboBoxElenco.getSelectedIndex() <= 0) {
            JOptionPane.showMessageDialog(this, "Selecione um Elenco válido!", "Aviso", JOptionPane.WARNING_MESSAGE);
            return;
        }
        
        if (jComboBoxFilme.getSelectedIndex() <= 0) {
            JOptionPane.showMessageDialog(this, "Selecione um Filme válido!", "Aviso", JOptionPane.WARNING_MESSAGE);
            return;
        }
        
        if (jComboBoxTipoElenco.getSelectedIndex() <= 0) {
            JOptionPane.showMessageDialog(this, "Selecione um Tipo de Elenco válido!", "Aviso", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        try {
            ...
            if (verificarDuplicado(idElenco, idFilme)) {
                JOptionPane.showMessageDialog(this, "Este ator já está cadastrado neste filme!", "Aviso", JOptionPane.WARNING_MESSAGE);
                return;
            }
            
            // SQL de inserção direta
    ...
    private boolean verificarDuplicado(int idElenco, int idFilme) {
        try {
            String sql = "SELECT COUNT(*) FROM atuacao_elenco WHERE ID_Elenco = ? AND ID_Filme = ?";
            ConexaoMySQL conexao = new ConexaoMySQL();
            Connection conn = conexao.conectar();
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setInt(1, idElenco);
    ...
    private void abrirConsulta() {
        InserirTipoElenco consulta = new InserirTipoElenco();
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(() -> new InserirTipoElenco().setVisible(true));
    }
